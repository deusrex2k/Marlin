...
inline void Draw_Prepare_Menu() {
  ...
  #endif
  #if HAS_HOTEND //TODO: add custom preheats
...
void Draw_Print_ProgressBar() {
  ...
  DWIN_Draw_IntValue(true, true, 0, font8x16, Percent_Color - (0x800*(Percentrecord/4)), Color_Bg_Black, 2, 117, 133, Percentrecord);//gradually change progress bar from yellow to green, i think*/
...
void update_variable() {
  ...
  if (last_speed != feedrate_percentage) {
    DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 3, 33 + 2 * STAT_CHR_W, 414, feedrate_percentage);
    last_speed = feedrate_percentage;
  }
  #if HAS_ZOFFSET_ITEM
    if (last_zoffset != BABY_Z_VAR) {
      DWIN_Draw_Signed_Float(DWIN_FONT_STAT, Color_Bg_Black, 2, 2, 178 + STAT_CHR_W, 414, BABY_Z_VAR * 100);
...
inline void Draw_SDItem(const uint16_t item, int16_t row=-1) {
  ...
  inline void Draw_SDItem_Shifted(int8_t &shift) {
    ...
    NOMORE(shift, _MAX(len - MENU_CHAR_LIMIT, 0U));//TODO: Display longer filenames
...
void Draw_Status_Area(const bool with_update) {
  ...
  DWIN_ICON_Show(ICON, ICON_Speed, 13, 414);
  DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 3, 33 + 2 * STAT_CHR_W, 414, feedrate_percentage);
  DWIN_Draw_String(false, false, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 33 + 5 * STAT_CHR_W + 2, 414, F("%"));

  #if HAS_ZOFFSET_ITEM
    DWIN_ICON_Show(ICON, ICON_Zoffset, 158, 413);
    DWIN_Draw_Signed_Float(DWIN_FONT_STAT, Color_Bg_Black, 2, 2, 178, 414, BABY_Z_VAR * 100);
  #endif
